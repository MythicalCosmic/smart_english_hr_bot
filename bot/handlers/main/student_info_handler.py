...
async def process_additional_courses(message: Message, state: FSMContext, user_lang: str = 'uz'):
    try:
        lang = await get_lang(state, user_lang)
        app_id = await get_app_id(state)
        ...
        print(f'Error on process_additional_courses: {e}')


@router.message(ApplicationState.additional_courses_subject, F.text)
async def process_additional_courses_subject(message: Message, state: FSMContext, user_lang: str = "uz"):
    try:
        lang = await get_lang(state, user_lang)
        ...
        app_id = await get_app_id(state)
        course_subject = message.text.strip()
        ...
        print(f"Error on process_additional_courses_subject: {e}")
