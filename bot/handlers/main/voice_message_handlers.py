...
from bot.validators.validator import is_back, get_english_level
    ...
    try:
        ...
        if is_back(message.text):
            if data.get("marriage_status"):
                ...
                await DB.user.set_state(message.from_user.id, ApplicationState.if_children.state)
            else:
        ...
        level = get_english_level(message.text)
        if not level:
